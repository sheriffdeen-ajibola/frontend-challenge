...
function CarouselDots({ className, ...props }: React.ComponentProps<"div">) {
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      try {
        ...
        setDots(api.scrollSnapList().map((_, i) => i));
      } catch {
        // ignore
      }
    };
  ...
  return (
    <div
      ...
      className={cn("mt-4 flex justify-center gap-3 items-center", className)}
      {...props}>
      {dots.map((_, idx) => (
        <button
          key={idx}
          ...
          className={cn(
            "w-2 h-2 rounded-full transition-colors border border-white/30",
